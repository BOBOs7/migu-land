...
export function HeroSection({ profile, onEnterWorks }: HeroSectionProps) {
  return (
    <section
      id="hero"
    ...
    >
      <div className="flex flex-1 flex-col items-center justify-center">
        ...
        <h1 className="mt-8 font-display text-display text-ink">{t(profile.name)}</h1>
        <p className="mt-4 max-w-xl text-body text-ink-muted">{t(profile.tagline)}</p>
      </div>
      ...
      {onEnterWorks && (
        <div className="pointer-events-none flex shrink-0 flex-col items-center gap-2 pb-8 pt-6 text-caption uppercase tracking-widest text-ink-muted">
          <span className="animate-pulse">向下滚动,查看项目</span>
